"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
Object.defineProperty(exports, "__esModule", { value: true });
// import { config } from 'dotenv';
var rq = require("request-promise-native");
var path = require("path");
var fs = require("fs");
exports.optionsTokenRequest = {
    // tslint:disable-next-line: max-line-length
    body: "{\"client_id\":\"" + process.env.AUTH0_CLIENT_ID + "\",\"client_secret\":\"" + process.env.AUTH0_CLIENT_SECRET + "\",\"audience\":\"" + process.env.AUTH0_AUDIENCE + "\",\"grant_type\":\"client_credentials\"}",
    headers: { 'content-type': 'application/json' },
    method: 'POST',
    url: process.env.AUTH0_REQ_URL,
    resolveWithFullResponse: true,
};
var isTokenOutdated = function (issuance_ms, issuance_duration_ms) {
    var now = new Date();
    if ((now.getTime() - issuance_ms) < issuance_duration_ms) {
        return true;
    }
    return false;
};
exports.readTokenFromDisc = function (filePath) { return __awaiter(_this, void 0, void 0, function () {
    var content, json, options, response, error_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 4, , 5]);
                if (!(fs.existsSync(filePath) === true)) return [3 /*break*/, 2];
                content = fs.readFileSync(filePath, 'utf8');
                json = JSON.parse(content);
                // the file does not have what we want
                if (json.hasOwnProperty('access_token') === false || json.hasOwnProperty('token_type') === false) {
                    // return undefined;
                    exports.getNewToken(filePath, exports.optionsTokenRequest);
                }
                if (isTokenOutdated(json.issance, json.expires_in * 1000) === true) {
                    exports.getNewToken(filePath, exports.optionsTokenRequest);
                }
                options = {
                    url: process.env.API_URL,
                    method: 'GET',
                    headers: { authorization: json.token_type + " " + json.access_token, Accept: 'application/json' },
                    resolveWithFullResponse: true,
                    json: true
                };
                return [4 /*yield*/, rq(options)];
            case 1:
                response = _a.sent();
                if (response.statusCode !== 200) {
                    exports.getNewToken(filePath, exports.optionsTokenRequest);
                }
                return [2 /*return*/, json];
            case 2: throw new Error('Wrong path');
            case 3: return [3 /*break*/, 5];
            case 4:
                error_1 = _a.sent();
                console.error(error_1);
                return [3 /*break*/, 5];
            case 5: return [2 /*return*/];
        }
    });
}); };
var writeTokenToDisk = function (filePath, dataStr) {
    console.info('writing token to disk');
    fs.writeFileSync(filePath, dataStr);
};
exports.getNewToken = function (filePath, opts) { return __awaiter(_this, void 0, void 0, function () {
    var response, parsedBody, error_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, rq(opts)];
            case 1:
                response = _a.sent();
                if (response.statusCode !== 200) {
                    console.log(response);
                    throw new Error('Status on new token request is not 200');
                }
                else {
                    parsedBody = JSON.parse(response.body);
                    parsedBody.issuance = new Date().getTime();
                    writeTokenToDisk(filePath, JSON.stringify(parsedBody));
                }
                return [3 /*break*/, 3];
            case 2:
                error_2 = _a.sent();
                console.error(error_2.message);
                throw error_2;
            case 3: return [2 /*return*/];
        }
    });
}); };
(function () { return __awaiter(_this, void 0, void 0, function () {
    var token, headers, options, response, error_3;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 3, , 4]);
                return [4 /*yield*/, exports.readTokenFromDisc(path.resolve(process.cwd(), './token.json'))];
            case 1:
                token = _a.sent();
                if (token === undefined) {
                    throw new Error('Token does not exists');
                }
                headers = { authorization: token.token_type + " " + token.access_token, Accept: 'application/json' };
                options = {
                    url: process.env.API_URL,
                    method: 'GET',
                    headers: headers
                };
                return [4 /*yield*/, rq(options)];
            case 2:
                response = _a.sent();
                // const response = await request(app).post('/default/post')
                //   .send({})
                //   .set(headers);
                console.log(response);
                return [3 /*break*/, 4];
            case 3:
                error_3 = _a.sent();
                console.error(error_3);
                throw error_3;
            case 4: return [2 /*return*/];
        }
    });
}); })();
//# sourceMappingURL=index.js.map