...
export const getUserById: (userId: string) => Promise<User> = async (userId) => {
  try {
    const userRepo = getRepository(User);
...
export const getUserByIdWithSpots: (userId: number) => Promise<User> = async (userId) => {
  try {
    const userRepo = getRepository(User);
    const query = userRepo.createQueryBuilder('user')
...
export const getUsersByRole: (role: string) => Promise<User[]> = async (role) => {
  try {
