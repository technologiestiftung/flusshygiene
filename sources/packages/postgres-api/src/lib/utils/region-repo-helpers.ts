...
export const findByName: (region: string) => Promise<Region> = async (
  region,
) => {
  try {
...
export const findByIdWithRelations: (
  regionId: number,
  relations: string[],
) => Promise<Region> = (regionId: number, relations: string[]) => {
  try {
    const repo = getRepository(Region);
...
const getNamesList: () => Promise<any[]> = async () => {
  const repo = getRepository(Region);
  try {
    const query = await repo.createQueryBuilder('regions').select('name');
    // console.log(query);
