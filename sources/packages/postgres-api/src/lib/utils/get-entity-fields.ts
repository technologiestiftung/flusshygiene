...
const getPropertyNames = async (enititySchema: string) => {
  return getConnection()
    .getMetadata(enititySchema)
    .ownColumns.map((column) => column.propertyName);
};

const getPropertTypeList = async (entitiySchema: string) => {
  return getConnection()
    .getMetadata(entitiySchema)
    .ownColumns.map((column) => [column.propertyName, column.type]);
};
...
export const getEntityFields: entityFields = async (type) => {
  ...
  try {
    ...
    filteredPropNames = propertyNames.filter(
      (ele) => notAllowedProps.includes(ele) !== true,
    );
  } catch (e) {
    throw e;
  }
